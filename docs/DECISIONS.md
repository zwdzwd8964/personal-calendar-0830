...
- 2026-08-30 · 设置页：导入仅校验 meals/tasks 为数组，另拒绝 schemaVersion 高于当前的文件；导出/清空按钮在 stores ready 前禁用；清空两步确认视觉升级（第二步红色 danger） · 与 storage.load 的严格度对齐；防 init 完成前误导出空数据。
